...
bool eat(ofxBox2dBaseShape A, ofxBox2dBaseShape B)
{
    if (A.rank > B.rank && A.hungerM < 300 && A.rank > Hrank_STANDARD && B.rank >0) {
    ...
    else if(A.rank > B.rank && A.rank==Hrank_STANDARD && B.rank==0 && A.hungerM < 300){
...
void testApp::update(){
    ...
    box2d.update();
    vector<ofxBox2dRect> _tempCEat;
    vector<ofxBox2dCircle> _tempHCEat;
    vector<ofxBox2dPolygon> _tempPHEat;
    ...
    for (vector<ofxBox2dCircle>::iterator it = herbivores.begin(); it!=herbivores.end(); ++it) {
        if (it->exist == false) {
            it->destroy();
            countH--;
        }else _tempHCEat.push_back(*it);
    }
    herbivores = _tempHCEat;
    
    //---------------collision detect and eat carnivores-------------------------------------
    ...
    for (vector<ofxBox2dRect>::iterator it = carnivores.begin(); it!=carnivores.end(); ++it) {
        if (it->exist == false) {
            it->destroy();
            countC--;
        }else _tempCEat.push_back(*it);
    }
    
    carnivores=_tempCEat;
    //----------------refresh the stack------------------------------------------------------
...
void testApp::touchDown(ofTouchEventArgs & touch){
    if(touch.id == 1)
    {
        ...
        int r = ofRandom(10,15);
        int x = touch.x;int y = touch.y;
        h.setup(box2d.getWorld(), x, y, r);
